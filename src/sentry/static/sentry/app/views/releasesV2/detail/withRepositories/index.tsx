...
const withRepositories = <P extends Props>(WrappedComponent: React.ComponentType<P>) =>
  class extends React.Component<P, State> {
    ...
    setActiveRepo(props: P) {
      const activeRepository = props.location.query?.activeRepo;
      ...
      if (activeRepository !== this.state.activeRepository) {
        this.setState(prevState => ({
          activeRepository:
            prevState.repositories.find(repo => repo.name === activeRepository) || null,
        }));
      }
    }
    ...
    async fetchRepositories() {
      ...
      try {
        ...
        addErrorMessage(
          t(
            `An error occured while trying to fetch the repositories of the release: ${release}`
          )
        );
      }
    }
    ...
    render() {
      ...
      if (isLoading || activeRepository === undefined) {
